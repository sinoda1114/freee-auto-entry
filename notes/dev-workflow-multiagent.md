...
- リポジトリの**実体ディレクトリ `~/dev/freee-auto-entry` は「main 統合＋デプロイ専用」**。
...
git fetch origin
git worktree add ../freee-auto-entry-<topic> -b feat/<topic> origin/main
# 起点ズレ確認（0 であること）
git -C ../freee-auto-entry-<topic> rev-list --count origin/main..HEAD

# 一覧 / 後片付け
git worktree list
git worktree remove ../freee-auto-entry-<topic>   # マージ後に撤去
```

- 命名規則: `feat/<topic>` `fix/<topic>` `chore/<topic>`。worktree dir は `../freee-auto-entry-<topic>`。
- **同じファイルを2つの worktree で同時編集しない**（役割境界を守る）。
- worktree は本体 repo の `node_modules` を symlink すると検証が速い（`ln -s ../freee-auto-entry/node_modules node_modules`）。
